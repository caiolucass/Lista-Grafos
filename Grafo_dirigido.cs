...
namespace Lista_pratica
{
    public class Grafo_dirigido : Grafo, Interface_grafo_dirigido
    {
        ...
        private void preencher_Grafo()
        {
            ...
            //percorre o arquivo à partir da linha "1", que contem a quantidade de vértices
            for (int i = 1; i < this.Arquivo.Length; i++)
            {
                ...
                // verfica se o vértice não existe
                if (Vert_1_aux == null)
                {
        ...
        public int get_GrauEntrada(Vertice v1)
        {
            ...
            {
                //percorre as arestas dos vértices adjacentes
                for (int x = 0; x < v1.Adjacente[i].Aresta.Count(); x++)
                {
                    //verifica se os vértices são iguais e se possuem a mesma direção
